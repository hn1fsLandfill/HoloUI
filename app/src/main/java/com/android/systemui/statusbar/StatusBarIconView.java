...
public class StatusBarIconView extends AnimatedImageView {
    ...
    public static Drawable getIcon(Context context, StatusBarIcon icon) {
        ...
        /*TODO if (icon.pkg != null) {
            try {
                int userId = icon.user.describeContents(); // .getIdentifier();
                if (userId == -1) {
                    userId = 0;
                }
                r = context.getApplicationContext().getPackageManager()
                        .getResourcesForApplicationAsUser(icon.pkg, userId);
            } catch (PackageManager.NameNotFoundException ex) {
                Log.e(TAG, "Icon package not found: " + icon.pkg);
                return null;
            }
        } else {
        ...
        try {
            return r.getDrawable(icon.icon.getResId());
        } catch (RuntimeException e) {
            Log.w(TAG, "Icon not found in "
                    + (icon.pkg != null ? icon.number : "<system>")
                    + ": " + Integer.toHexString(icon.number));
        } */
