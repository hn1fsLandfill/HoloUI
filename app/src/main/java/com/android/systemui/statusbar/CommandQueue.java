...
import com.android.systemui.shims.IStatusBar_Stub;
 ...
 */
public class CommandQueue extends IStatusBar_Stub {
    ...
    public void setImeWindowStatus(int displayId, int vis, int backDisposition, boolean showImeSwitcher) {
        synchronized (mList) {
            ...
            mHandler.obtainMessage(MSG_SHOW_IME_BUTTON, vis, backDisposition)
                    .sendToTarget();
        }
    }
    ...
    @Override
    public void setWindowState(int display, int window, int state) {
        synchronized (mList) {
